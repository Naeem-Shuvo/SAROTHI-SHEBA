...
// *   **If [exp]is a number**: It calculates [exp * 1000]. JWT expiration times are in seconds, but JavaScript's `Date.now()` uses milliseconds, so this converts it to the correct unit.
// *   **If [exp]is NOT a number** (meaning it wasn't passed correctly): It creates a fallback expiration time of **one hour from now**. This ensures the blacklisted token will eventually be cleaned up even if its original expiration time was lost.
